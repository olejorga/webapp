...
export default async function handler(
  ...
  switch (filter) {
    case 'gender':
      const studentsByGender = await prisma.student.findMany({
        orderBy: { gender: 'asc' },
      })
      if (req.method === 'GET') {
        if (studentsByGender) {
    ...
    case 'age':
      const studentsByAge = await prisma.student.findMany({
        orderBy: { age: 'asc' },
      })
      if (req.method === 'GET') {
        if (studentsByAge) {
    ...
    case 'group':
      const studentsByGroup = await prisma.student.findMany({
        orderBy: { group: 'asc' },
      })
      if (req.method === 'GET') {
        if (studentsByGroup) {
